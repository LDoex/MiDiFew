...
Del_cols = np.array(['comm_write_fun', 'command_length',  'gain', 'reset', 'deadband', 'cycletime',
                      'rate', 'crc_rate'])
Continious_cols = np.array(['command_memory_count', 'response_memory_count',
                           'measurement', 'time'])
Discrete_cols = np.array(['command_address', 'response_address', 'command_memory', 'response_memory',
                         'comm_read_function', 'resp_read_fun', 'resp_write_fun', 'sub_function',
                         'resp_length', 'setpoint', 'control_mode', 'control_scheme', 'pump', 'solenoid'])
...
for i in Discrete_cols:
    ...
    Discrete_Traindata_onehot = curTrain if Discrete_Traindata_onehot is None else pd.concat([Discrete_Traindata_onehot, curTrain],axis = 1)
    Discrete_Testdata_onehot = curTest if Discrete_Testdata_onehot is None else pd.concat([Discrete_Testdata_onehot, curTest],axis = 1)
...
# #save the total data
# Train_data.to_csv('./csvData/Pipeline_Train_onehot.csv', index=False)
# Test_data.to_csv('./csvData/Pipeline_Test_onehot.csv', index=False)

#split by class
Train_dir = './csvData/PipelineTrainClassData_onehot/'
Test_dir = './csvData/PipelineTestClassData_onehot/'
