...
File_names = {str(i): os.path.join('./csvData/NewGaspipeline_'+str(i)+'.csv') for i in range(8)}
TrainOutput_path = os.path.join('./csvData/NewGaspipeline_Train.csv')
TestOutput_path = os.path.join('./csvData/NewGaspipeline_Test.csv')
...
for key, df in DataFrames.items():
    Trains[key], Tests[key] = train_test_split(df, train_size=0.5, random_state=0)
