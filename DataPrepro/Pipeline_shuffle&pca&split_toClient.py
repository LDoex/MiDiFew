...
Train_file = pd.read_csv('./csvData/Pipeline_Train_onehot.csv')
Test_file = pd.read_csv('./csvData/Pipeline_Test_onehot.csv')
...
TrainOutput_paths = {str(i): os.path.join('./csvData/Pipeline_client_data/client{}'.format(i))
                     for i in range(client_num)}
TestOutput_paths = {str(i): os.path.join('./csvData/Pipeline_client_data/client{}'.format(i))
                     for i in range(client_num)}
...
for i in range(client_num):
    ...
    Train_data, Test_data = norm_util.norm(Train_data, Test_data, 4)
