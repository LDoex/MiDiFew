...
col_names = np.array(['command_address', 'response_address', 'command_memory', 'response_memory',
                      ...
                      'crc_rate', 'measurement', 'time', 'label'])

# 定义pipeline数据预处理函数, 按分类转换成csv
def preHandel_data():
    source_file = './rawData/Gaspipeline.txt'
    handled_files = {str(i): os.path.join('./csvData/Pipeline_'+str(i)+'.csv') for i in range(8)}
