...
def random_list(start,stop,length,internal):
    if length>=0:
        length=int(length)
    ...
    for i in range(length):
        cur_ind = random.randint(start+(i+1)*internal, start+(i+1)*internal+5000)
        while cur_ind>stop:
            cur_ind = random.randint(start + (i + 1) * internal, start + (i + 1) * internal + 5000)

        a = abs(cur_ind-random_list[-1])
        b = abs(random_list[0]-cur_ind)
        while random_list is None and abs(cur_ind-random_list[-1])<8000 and abs(random_list[0]-cur_ind)<8000:
...
Train_inds = sorted(random_list(0, Train_shuffle.shape[0], client_num-1, num_perTrain))
Test_inds = sorted(random_list(0, Test_shuffle.shape[0], client_num-1, num_perTest))

Train_inds.append(Train_shuffle.shape[0])
Test_inds.append(Test_shuffle.shape[0])

for i in range(client_num):
    print("cur_client{},Train{},Test{}".format(i, Train_inds[i+1]-Train_inds[i], Test_inds[i+1]-Test_inds[i]))
