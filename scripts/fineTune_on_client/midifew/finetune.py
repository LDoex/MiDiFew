...
from MiDiFewNets.finetune_engine import Engine
...
def main(opt):
    ...
    # save opts
    with open(os.path.join(opt['log.exp_dir'], opt['log.best_name'] + '_' + opt['train.clientName'] + '_opt.json'),
              'w') as f:
        json.dump(opt, f)
        f.write('\n')
    ...
    model = transfer_model.extend_param(model)
